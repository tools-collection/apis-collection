# 📦 [Coinbase](https://developers.coinbase.com/)


> **Coinbase’s API makes it easy to integrate bitcoin, bitcoin cash, litecoin and ethereum into both new and existing applications. 
Coinbase’s APIs enable a variety of capabilities; from simply gathering read-only data, to building something that’s never been done before**

## 📝 Details
- **Categories**: [Payments](https://github.com/tools-collection/apis-collection#payments-), [Blockchain & Cryptocurrencies](https://github.com/tools-collection/apis-collection#blockchain--cryptocurrencies-)
- **Type**: `REST`
- **Auth**: `none`
- **Free**: `No`
- **Status**: `Graveyard` 🪦

## 🚦 Rate Limits
*Not specified*

## 📖 Specification
*Not specified*

## 🔗 Links
- [Docs / Website](https://developers.coinbase.com/)

## 🧰 Libraries
### 🛠️ ccex-api TypeScript SDK by Dang Thanh Tung
- **Platform**: `JavaScript, Node.js`
- **Docs**: [📚 Documentation](https://github.com/dang1412/ccex-api)


- **Author**: 👤 dang1412
- **Stars**: ⭐ 30
### 🛠️ ExchangeSharp .NET/C# by Jeff Johnson
- **Platform**: `.Net, C#`
- **Docs**: [📚 Documentation](https://github.com/jjxtra/ExchangeSharp)


- **Author**: 👤 jjxtra
- **Stars**: ⭐ 759
### 🛠️ Coinbase .Net SDK by Youssef Boukenken
- **Platform**: `.Net, C#`
- **Docs**: [📚 Documentation](https://github.com/sefbkn/Coinbase.NET/blob/master/README.md)
- **Source**: [💻 Source Code](https://github.com/sefbkn/Coinbase.NET)

- **Author**: 👤 sefbkn
- **Stars**: ⭐ 4
### 🛠️ Coinbase .Net SDK by Brian Chavez
- **Platform**: `.Net, C#`
- **Docs**: [📚 Documentation](https://developers.coinbase.com/docs/wallet/client-libraries)
- **Source**: [💻 Source Code](https://github.com/bchavez/Coinbase)

- **Author**: 👤 bchavez
- **Stars**: ⭐ 184
### 🛠️ Coinbase Python SDK by Coinbase
- **Platform**: `Python`
- **Docs**: [📚 Documentation](https://developers.coinbase.com/api/v2#official-client-libraries)
- **Source**: [💻 Source Code](https://github.com/coinbase/coinbase-python)


- **Stars**: ⭐ 542

## 📬 Contact
*Not provided*

## 💬 Discussion
*Not provided*

---
[Back to README](../../README.md) | Supported by [Online Tools Collection](https://tools-collection.net)
