# 📦 [Airports Web](https://airport-web.appspot.com/api/docs/)


> **Covered Airports mostly Germany. Request Airport name and website URL for airports**

## 📝 Details
- **Categories**: [Other](https://github.com/tools-collection/apis-collection#other-)
- **Type**: `N/A`
- **Auth**: `none`
- **Free**: `No`
- **Status**: `Graveyard` 🪦

## 🚦 Rate Limits
*Not specified*

## 📖 Specification
- **Type**: `swagger`
- **URL**: [https://airport-web.appspot.com/api/docs/swagger.json](https://airport-web.appspot.com/api/docs/swagger.json)
- **Version**: `https://airport-web.appspot.com/api/docs/swagger.json`

## 🔗 Links
- [Docs / Website](https://airport-web.appspot.com/api/docs/)

## 🧰 Libraries
*No libraries available*

## 📬 Contact
📧 support@adyen.com

## 💬 Discussion
*Not provided*

---
[Back to README](../../README.md) | Supported by [Online Tools Collection](https://tools-collection.net)
