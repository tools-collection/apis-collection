# 📦 [Eventful API](http://api.eventful.com/)


> **Eventful is a leading digital media company connecting consumers with entertainment, movies and local events. Concert &amp; Event API Eventful is the world&#x27;s largest collection of events, taking place in local markets throughout the world, from concerts and sports to singles events and political rallies. Event. Is a Paid Usage model and requires a licensing agreement directly with Eventful**

## 📝 Details
- **Categories**: [Events](https://github.com/tools-collection/apis-collection#events-), [Point of Interest](https://github.com/tools-collection/apis-collection#point-of-interest-), [Music & Audio](https://github.com/tools-collection/apis-collection#music--audio-), [Calendar & Time](https://github.com/tools-collection/apis-collection#calendar--time-)
- **Type**: `N/A`
- **Auth**: `none`
- **Free**: `No`
- **Status**: `Graveyard` 🪦

## 🚦 Rate Limits
*Not specified*

## 📖 Specification
*Not specified*

## 🔗 Links
- [Docs / Website](http://api.eventful.com/)

## 🧰 Libraries
*No libraries available*

## 📬 Contact
*Not provided*

## 💬 Discussion
*Not provided*

---
[Back to README](../../README.md) | Supported by [Online Tools Collection](https://tools-collection.net)
